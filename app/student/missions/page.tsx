    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col gap-2">
                <h1 className="text-4xl font-black text-white tracking-tight">Misiones Disponibles</h1>
                <p className="text-gray-400">Completa tus desafíos para desbloquear nuevas habilidades.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {assignments.length > 0 ? (
                    assignments.map(a => (
                        <div key={a.assignmentId} className="p-6 rounded-3xl bg-[#252525] border border-white/5 hover:border-primary/50 transition-all group">
                            <div className="flex justify-between items-start mb-4">
                                <span className="material-symbols-outlined text-3xl text-primary group-hover:scale-110 transition-transform">rocket_launch</span>
                            ...
                            <p className="text-sm text-gray-500 line-clamp-2 mb-6">{a.subject}</p>
                            ...
                            <a href={`/assessment/${a.examId}`} className="block w-full py-3 bg-white text-black font-black text-center rounded-xl hover:bg-primary hover:text-white transition-colors">
                                {a.status === 'COMPLETED' ? 'VER RESULTADOS' : 'INICIAR MISIÓN'}
                            </a>
                        </div>
                    ))
                ) : (
